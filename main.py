...
def summarize_schedule_for_button() -> str:
    ...
    for code in slot_order():
        ...
        lines.append(
            f"{sl['title']} | занято: {taken}/{CAPACITY} | свободно: {left}\n\n"
        )
    lines.append("")
                ...
                if not is_admin:
                    ...
                    if msg in _active_times():
                        ...
                        if not code:
                            send_msg(
                                user_id,
                                "Не удалось определить слот. Попробуйте ещё раз.",
                                kb=user_keyboard(),
                            )
                            continue
                        ...
                        if already_booked_count(fullname_me) >= MAX_SLOTS_PER_USER:
                            send_msg(
                                user_id,
                                "У вас уже есть запись. Используйте «Перезапись».",
                                kb=user_keyboard(),
                            )
                            continue
                        if len(users) >= CAPACITY:
                            send_msg(
                                user_id,
                                f"Лимит слота ({CAPACITY}) исчерпан.",
                                kb=user_keyboard(),
                            )
                            continue
                    ...
                    if msg == "Перезапись":
                        ...
                        send_msg(
                            user_id,
                            "Ваши записи очищены. Нажмите «Выбрать», чтобы записаться заново.",
                            kb=user_keyboard(),
                        )
                        continue
                    ...
                    if msg == "Подробно":
                        send_msg(
                            user_id,
                            full_schedule_text(show_lists=True),
                            kb=schedule_keyboard(),
                        )
                        continue
